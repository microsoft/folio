...
class FixturesImpl<WorkerFixtures, TestFixtures, WorkerParameters, W, T, P> {
  ...
  init(name: string, fixture: (params: any, runTest: (value: any) => Promise<void>) => Promise<void>, options?: FixtureOptions): void {
    if (this._finished)
  ...
  override(name: string, fixture: (params: any, runTest: (value: any) => Promise<void>) => Promise<void>): void {
    if (this._finished)
      throw new Error(`Should not modify fixtures after build()`);
    this._pool.overrideFixture(name as string, fixture as any);
  }

  initParameter<N extends keyof P>(name: N, description: string, defaultValue: P[N]): void {
    if (this._finished)
      throw new Error(`Should not modify fixtures after build()`);
...
const proxyHandler: ProxyHandler<FixturesImpl<any, any, any, any, any, any>> = {
  get: (target, prop, receiver) => {
    ...
    if (typeof prop !== 'string' || prop === 'then')
      return undefined;
    return {
      initParameter: (description, defaultValue) => target.initParameter(prop as any, description, defaultValue),
      init: (fn, options) => target.init(prop as any, fn, options),
      override: fn => target.override(prop as any, fn),
    };
  },
};
